  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 py-8 min-h-screen flex flex-col">
      ...
      <div className="flex flex-col mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-brass/20 pb-4">
          <div className="flex items-center gap-4">
            <Link href={`/boards/${encodeURIComponent(board)}`} className="inline-flex items-center justify-center text-celestial-blue hover:text-brass-dark transition-colors group" aria-label="Back to board">
              <ArrowLeftIcon className="w-8 h-8 group-hover:-translate-x-1 transition-transform" />
            </Link>
            <h1 className="text-3xl md:text-4xl font-serif font-bold text-starlight uppercase tracking-wider">
              {drawing.replace(/-/g, ' ')}
            </h1>
          </div>
          <span className="text-xs font-mono text-starlight mt-2 md:mt-0 uppercase">
            DRAWING_VIEWER_V1.0
          </span>
        </div>
      </div>

      <div className="flex-1 bg-deep-space overflow-hidden relative">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-celestial-blue via-celestial-blue to-celestial-blue opacity-50"></div>
