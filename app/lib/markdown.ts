...
renderer.code = function ({ text, lang }: { text: string; lang?: string }) {
  ...
  return `
    <div class="relative group my-6 border border-brass/20 bg-deep-space shadow-[4px_4px_0_0_rgba(var(--brass),0.2)] rounded-sm">
      <div class="flex justify-between items-center px-3 py-1 border-b border-brass/20 bg-void-black/50">
        <span class="text-xs font-mono text-celestial-blue uppercase">${language || 'CODE'}</span>
        <button 
          class="copy-btn text-starlight hover:text-brass-dark transition-colors"
...
renderer.link = function ({ href, title, text }: { href: string; title?: string | null; text: string }) {
  ...
  return `<a href="${href}"${titleAttr} class="text-celestial-blue hover:text-brass-dark hover:underline decoration-brass-dark underline-offset-4 transition-all break-all">${text}</a>`;
};
...
export async function processMarkdown(content: string): Promise<string> {
  ...
  doc.querySelectorAll('.obsidian-link').forEach((link: any) => {
    link.className = '!text-brass !no-underline hover:!text-brass-dark hover:!underline !decoration-brass-dark !underline-offset-4 !transition-all !break-all';
  });
  ...
  doc.querySelectorAll('table').forEach((table: any) => {
    ...
    wrapper.className = 'not-prose overflow-x-auto w-full my-8 bg-deep-space border border-brass/20 shadow-lg pb-2 custom-scrollbar';

    // Update table classes and styles for uniformity
    table.classList.add('min-w-full', 'divide-y', 'divide-brass/20');
    ...
    // Style Headers
    table.querySelectorAll('thead').forEach((thead: any) => thead.classList.add('bg-void-black'));
    table.querySelectorAll('th').forEach((th: any) => {
      th.className = 'px-6 py-4 text-left text-xs font-mono text-celestial-blue uppercase tracking-wider border-b border-brass/20';
    });

    // Style Body Cells
    table.querySelectorAll('tbody').forEach((tbody: any) => tbody.classList.add('divide-y', 'divide-brass/20', 'bg-deep-space'));
  ...
  doc.querySelectorAll('blockquote').forEach((blockquote: any) => {
    ...
    if (match) {
      ...
      let colorClass = 'border-celestial-blue text-celestial-blue';
      ...
      switch (type) {
        ...
        case 'summary':
        case 'tldr':
          colorClass = 'border-celestial-blue text-celestial-blue';
        ...
        case 'todo':
        case 'note':
          colorClass = 'border-brass text-brass';
        ...
        case 'help':
        case 'faq':
          colorClass = 'border-brass-dark text-brass-dark';
        ...
        case 'quote':
        case 'cite':
          colorClass = 'border-starlight text-starlight';
      ...
      container.className = `my-6 border-l-4 ${colorClass} bg-deep-space/30 rounded-r-md overflow-hidden shadow-lg`;

      const header = doc.createElement('div');
      header.className = `flex items-center gap-2 px-4 py-2 font-bold bg-void-black/20 ${colorClass}`;
